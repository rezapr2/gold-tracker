...
/**
 * RPC-backed settings: pulls the raw `bot_settings` doc from core over RabbitMQ
 * (cached ~15s) and applies the same DB⊕env precedence as everywhere else;
 * {@link invalidate} is called on a `settings.changed` event for instant refresh.
 */
@Injectable()
