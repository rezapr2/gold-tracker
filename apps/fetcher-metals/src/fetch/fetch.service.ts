...
export class FetchService implements OnApplicationBootstrap {
  ...
  // Assets this fetcher owns: everything the primary USD provider serves (XAU, XAG).
  private readonly assets: Asset[] = assetsForProvider('goldapicom');
  ...
  async fetchPrices(): Promise<void> {
    const run = async () => {
      await mapWithConcurrency(this.assets, FETCH_CONCURRENCY, async (asset) => {
        try {
